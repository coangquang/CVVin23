    ...
    for i in tqdm(range(len(test_salobj_dataset))):
        with torch.no_grad():
            ...
            if opt.model == 'u2net':
                ...
                prediction = result[0]
            total_mae += mae(prediction.cpu().numpy(), labels_v.cpu().numpy())
            total_wfm += wfm(prediction.cpu().numpy(), labels_v.cpu().numpy())
            total_sm += sm(prediction.cpu().numpy(), labels_v.cpu().numpy())
